 ...
 *
 * Une fois par appareil : un club se connecte tous les matins, et une
 * animation de quatre secondes tous les matins devient une corvee. Un clic
...
/** Memoire longue : le splash ne se joue qu'une fois par appareil. */
export const WELCOME_SHOWN = 'welcome_shown'
...
export default function WelcomeSplash() {
  ...
  /** Le drapeau est CONSOMME : deux montages ne rejouent pas l'animation. */
  const tryPlay = useCallback(() => {
    let flagged = false
    try {
      flagged = sessionStorage.getItem(JUST_LOGGED_IN) === '1'
      if (flagged) sessionStorage.removeItem(JUST_LOGGED_IN)
      if (flagged && localStorage.getItem(WELCOME_SHOWN)) flagged = false
      if (flagged) localStorage.setItem(WELCOME_SHOWN, '1')
    } catch {
