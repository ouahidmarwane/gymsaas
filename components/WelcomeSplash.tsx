    ...
    <div className={`welcome-splash${hiding ? ' hide' : ''}`}
      ...
      <div className="welcome-stage">
        ...
        {/*
          ...
          Elles montent depuis le bas, marquent leur point haut, puis
          retombent. Aucune ne touche au trace : la version precedente
          enfermait une COPIE du mot deformee par un bruit fractal, et les
          lettres vues au travers devenaient tremblees — une ecriture
          d'enfant la ou le mot doit rester net. Ici le verre ne fait que
          flouter ce qui passe derriere lui, ce qui se lit comme de la
          matiere sans rien tordre.

          Les parametres sont ecrits a la main, pas tires au sort : un
          `Math.random()` au rendu donnerait des valeurs differentes sur le
          serveur et dans le navigateur, et React signalerait une hydratation
          divergente. Le desordre est donc choisi une fois pour toutes.
        */}
        <span className="welcome-rain">
          {DROPS.map((d, i) => (
            <span key={i} className="welcome-drop"
                  style={{
                    // Sans unite : la feuille les multiplie par --stage-w,
                    // donc la pluie se met a l'echelle du mot.
                    '--x': `${d.x}%`,
                    '--size': d.size,
                    '--drift': `${d.drift}px`,
                    '--peak': d.peak,
...
/**
 * Les gouttes, une ligne par bulle.
 *
 * `x` place la bulle sur la largeur du mot, `peak` dit jusqu'ou elle monte,
 * `drift` de combien elle derive lateralement, `lag` quand elle part.
 *
 * Les retards s'etalent sur toute la duree du trace : les premieres montent
 * pendant que le mot s'ecrit, les dernieres pendant la pause. Rien n'est
 * regulier — des tailles et des hauteurs identiques donneraient une rangee,
 * pas une pluie.
 */
const DROPS = [
  { x: 8,  size: 0.055, drift: -12, peak: 0.14, dur: 2100, lag: 60 },
  { x: 21, size: 0.035, drift: 9,   peak: 0.19, dur: 2400, lag: 320 },
  { x: 32, size: 0.074, drift: -7,  peak: 0.12, dur: 1950, lag: 150 },
  { x: 44, size: 0.029, drift: 14,  peak: 0.21, dur: 2550, lag: 520 },
  { x: 53, size: 0.048, drift: -16, peak: 0.16, dur: 2250, lag: 240 },
  { x: 64, size: 0.065, drift: 6,   peak: 0.13, dur: 2050, lag: 700 },
  { x: 75, size: 0.039, drift: -10, peak: 0.20, dur: 2450, lag: 430 },
  { x: 86, size: 0.058, drift: 12,  peak: 0.15, dur: 2200, lag: 120 },
  { x: 94, size: 0.032, drift: -5,  peak: 0.22, dur: 2600, lag: 610 },
]
