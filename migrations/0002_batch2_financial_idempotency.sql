-- Batch 2: idempotence financiere et unicite des periodes facturees.
--
-- Les doublons historiques ne sont jamais supprimes. La revendication
-- canonique ci-dessous choisit une facture existante par periode et empeche
-- uniquement de nouvelles emissions concurrentes.
...
SELECT i.org_id, COALESCE(date(i.period_start), i.period_start),
       COALESCE(date(i.period_end), i.period_end), i.id, i.amount_cents, i.due_date,
  ...
  JOIN (
    SELECT org_id, COALESCE(date(period_start), period_start) AS period_start_day,
      ...
      FROM org_invoices GROUP BY org_id, period_start_day, period_end_day
